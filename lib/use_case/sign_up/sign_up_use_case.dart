...
abstract class ISignUpUseCase {
  Stream<BaseUseCaseResponse> invoke({required String email, required String password, required String name});
}
...
class SignUpUseCase extends ISignUpUseCase {
  ...
  @override
  Stream<BaseUseCaseResponse> invoke({required String email, required String password, required String name}) async* {
    try {
      yield BaseUseCaseResponse(isLoading: true);
      await FirebaseAuth.instance.createUserWithEmailAndPassword(email: email, password: password);
      await FirebaseAuth.instance.currentUser?.sendEmailVerification();
      await _userRepo.createUser(name: name, email: email, id: FirebaseAuth.instance.currentUser?.uid ?? "");
